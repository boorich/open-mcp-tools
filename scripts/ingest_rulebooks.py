...
def _detect_llm_provider() -> Optional[str]:
    """Auto-detect which LLM provider has an API key available"""
...
def ingest_pdf(pdf_path: Path, output_dir: Path, use_llm: bool = True, llm_provider: Optional[str] = None, overwrite: bool = False) -> Path:
    ...
    if use_llm:
        ...
        if llm_provider:
            logger.info(f"Attempting LLM-based parsing with {llm_provider}...")
    ...
    # Store source PDF path (relative to project root)
    project_root = Path(__file__).parent.parent
    try:
        pdf_path_relative = pdf_path.relative_to(project_root)
        resource_data["source_pdf"] = str(pdf_path_relative)
    except ValueError:
        # If PDF is outside project root, store absolute path
        resource_data["source_pdf"] = str(pdf_path.absolute())
