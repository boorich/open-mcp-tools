#!/usr/bin/env python3
"""
Open MCP Tools Server - Model Context Protocol Implementation
...
from open_mcp_tools import tools  # noqa: F401
from open_mcp_tools.core import RequestManager, get_registry
from open_mcp_tools.core.responses import (
    ErrorCodes,
    PromptResponse,
    ResourceResponse,
    Response,
)
from open_mcp_tools.core.types import JSONRPCRequest
from open_mcp_tools.handlers import (
    handle_cancelled,
    handle_initialize,
    handle_initialized,
    handle_ping,
...
from open_mcp_tools.handlers.resource_handler import (
    handle_resources_list,
    handle_resources_read,
)
from open_mcp_tools.payment_handler import (
...
from open_mcp_tools.utils.conversion import convert_keys_to_snake_case
...
async def lifespan(app: FastAPI):
    """Startup and shutdown logic for the Open MCP Tools server"""
    # Startup
    logger.info("Starting Open MCP Tools Server...")

    # Check if we're in development mode (hot-reload enabled)
    import os
    enable_hot_reload = os.getenv("OPEN_MCP_TOOLS_HOT_RELOAD", "false").lower() == "true"
    
    # Load canonical resources
    from open_mcp_tools.core.resources.loader import load_resources
    ...
    from open_mcp_tools.core.resources.registry import get_registry as get_resource_registry
    ...
    broadcast_task = None
    from open_mcp_tools.core.dcap import is_dcap_enabled, broadcast_all_tools
    from open_mcp_tools.env import get_env, get_env_int
    ...
    if enable_hot_reload:
        from open_mcp_tools.core.resources.loader import stop_hot_reload
        stop_hot_reload()
...
async def root():
    ...
    return {
        "name": "Open MCP Tools Server",
        "version": "0.1.0",
        "mcp_endpoint": "/mcp",
        "health_endpoint": "/health",
        "transport": "streamable-http",
        "streaming_format": "sse",
        "description": "General-purpose MCP server with extensible tool modules",
    }
