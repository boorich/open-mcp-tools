...
Loads canonical resources from YAML files.
        ...
        try:
            ...
            content = {k: v for k, v in data.items() 
                      if k not in ['name', 'version', 'category', 'description', 
                                 'tags', 'author', 'created_at', 'updated_at', 'source_pdf']}
            
            # Store source_pdf separately if present
    ...
    def _reload_single_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            if data and 'name' in data:
                ...
                uri = f"open-mcp-tools://{category_name}/{resource_name}/v{data.get('version', '1.0')}"
