...
from .base import CanonicalResource, ResourceCategory, parse_resource_uri

logger = logging.getLogger(__name__)


class ResourceRegistry:
    """Registry for canonical resources"""
    ...
    def get_resource_by_components(self, category: str, name: str, version: str) -> Optional[CanonicalResource]:
        ...
        Returns:
            Resource if found, None otherwise
        """
        uri = f"open-mcp-tools://{category}/{name}/v{version}"
        return self.get_resource(uri)
    
    def validate_uri(self, uri: str) -> bool:
        """
        Validate that a URI follows the open-mcp-tools://* scheme.
        ...
        """
        return parse_resource_uri(uri) is not None
